...
dashboardPage(skin = 'black',
    ...
    dashboardBody(
        ...
        tags$style(HTML("        /* --- NOVO CSS PARA O LAYOUT GLOBAL 01dez25 --- */
          ...
          .btn-success-run { background-color: #11ff11; border-color: black, color: white;}
          /* Alignment fix for buttons next to inputs */
          .align-btn { margin-top: 25px; } 
        "))
        ),
        
        # --- PAINEL DE CONTROLE GLOBAL (FLUTUANTE) ----
    ...
    tabItems(
        ...
        # === NOVO TAB: Data Processing Pipeline (Visão 3 Colunas) ====
        tabItem("data_processing",
                # fluidRow(
                #     h2("Data Processing: Map | Congruence (Cs) | Network Analysis (SCAN)"),
                #     tags$hr()
                # ),
